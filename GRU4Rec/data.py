...
class Data:
	def __init__(self, args):
		...
		self.negative_length = args.neg_samples
	...
	def get_input_data(self):
		self.train_sequences, self.train_items = [], []
		self.valid_sequences, self.valid_items = [], []
		self.test_sequences, self.test_items = [], []
		valid_test_num = 0
		...
		for u in np.random.permutation(self.user_size):
			...
			# train data
			for seq in slide_window(self.train_set[u], self.sequence_length+1):
				self.train_sequences.append(seq[:-1])
				self.train_items.append(seq[-1:] + random.sample(sample_pool, self.negative_length))
