...
def main():
	...
	parser.add_argument('--horizontal_filter_num', type=int, default=16)
	parser.add_argument('--vertical_filter_num', type=int, default=4)
...
def train(args, data, model):
	batch_users = tf.placeholder(tf.int32, [None, ])
	batch_sequences = tf.placeholder(tf.int32, [None, args.sequence_length])
	batch_items = tf.placeholder(tf.int32, [None, None])
	loss, train_op = model.forward(batch_users, batch_sequences, batch_items, True)
	predictions = model.forward(batch_users, batch_sequences, batch_items, False)
	...
	with tf.Session() as sess:
		...
		for i in range(1, args.epoch_num+1):
			...
			for j in range(1, train_iterations+1):
				...
				train_feed_dict = {batch_users: data.train_users[start:end], 
							batch_sequences: data.train_sequences[start:end], 
			...
			if i % 1 == 0:
				...
				valid_feed_dict = {batch_users: data.valid_users, batch_sequences: data.valid_sequences, batch_items: data.valid_items}
				...
				test_feed_dict = {batch_users: data.test_users, batch_sequences: data.test_sequences, batch_items: data.test_items}
				...
				if i - bepoch >= 20:
					...
					print(args.logid, ':', args.sequence_length, args.target_length, args.sample_ratio, args.horizontal_filter_num, args.vertical_filter_num, '\t', 
						args.epoch_num, args.batch_size, args.learn_rate, args.latent_dimension, args.keep_prob)
