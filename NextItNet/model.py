...
class NextItNet_Decoder:
	def __init__(self, model_para):
		...
		self.allitem_embeddings = tf.get_variable('allitem_embeddings', [model_para['item_size'], embedding_width], initializer = tf.truncated_normal_initializer(stddev = 0.02))
	...
	def train_graph(self, is_negsample = False):
		...
		if is_negsample:
			...
			logits = residual.conv1d(tf.nn.relu(dilate_input), model_para['item_size'], name = 'logits')
	...
	def model_graph(self, itemseq_input, train = True):
		model_para = self.model_para
		context_seq = itemseq_input[:, 0:-1] if train else itemseq_input
		label_seq = itemseq_input[:, 1:]

		context_embedding = tf.nn.embedding_lookup(self.allitem_embeddings, context_seq, name = "context_embedding")
		dilate_input = context_embedding
		for layer_id, dilation in enumerate(model_para['dilations']):
			dilate_input = residual.nextitnet_residual_block_one(dilate_input, dilation, layer_id, model_para['dilated_channels'], model_para['kernel_size'], causal = True, train = train)
	...
	def predict_graph(self, is_negsample = False,reuse = False):
		...
		dilate_input = tf.expand_dims(dilate_input[:, -1, :], 1)
		model_para = self.model_para
