...
def nextitnet_residual_block(input_, dilation, layer_id, residual_channels, kernel_size, causal = True, train = True):
	...
	with tf.variable_scope(resblock_name):
		dilated_conv = conv1d(input_, residual_channels, dilation, kernel_size, causal = causal, name = "dilated_conv1")
		...
		dilated_conv = conv1d(relu1,  residual_channels, dilation, kernel_size, causal = causal, name = "dilated_conv2")
...
def nextitnet_residual_block_one(input_, dilation, layer_id, residual_channels, kernel_size, causal = True, train = True):
		resblock_type = "decoder"
		...
		with tf.variable_scope(resblock_name):
			...
			dilated_conv = conv1d(relu2, int(0.5 * residual_channels), dilation, kernel_size, causal = causal, name = "dilated_conv")
			#dilated_conv = [conv1d(relu2, int(0.5 * residual_channels), dilation, i+1, causal = causal, name = "dilated_conv_%d" % (i+1)) for i in range(19)]
			#dilated_conv = tf.concat(dilated_conv, 2)
...
def nextitnet_residual_block_gatedCNN(input_, dilation, layer_id, residual_channels, kernel_size, causal = True, train = True):
	resblock_type = "decoder"
	resblock_name = "gatedCNN_{}_layer_{}_{}".format(resblock_type, layer_id, dilation)
	
	with tf.variable_scope(resblock_name):
		dilated_conv = conv1d(input_, residual_channels, dilation, kernel_size, causal = causal, name = "dilated_conv")
		tanh = tf.nn.tanh(dilated_conv)
		
		gate_conv = conv1d(input_, residual_channels, dilation, kernel_size, causal = causal, name = "gate_conv")
