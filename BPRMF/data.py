...
class Data:
	...
	def get_input_data(self):
		self.train_triples = []
		self.valid_users, self.valid_items = [], []
		self.test_users, self.test_items = [], []
		valid_test_num = 0
		...
		for u in np.random.permutation(self.user_size):
			...
			# train data
			for i in self.train_set[u]:
				j = random.sample(sample_pool, 1)[0]
				self.train_triples.append([u,i,j])
