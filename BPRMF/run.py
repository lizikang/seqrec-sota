...
def train(args, data, model):
	...
	batch_triples = tf.placeholder(tf.int32, [None, 3])
	batch_users = tf.placeholder(tf.int32, [None, ])
	batch_items = tf.placeholder(tf.int32, [None, 501])
	loss, train_op = model.forward(batch_triples, None, None, True)
	predictions = model.forward(None, batch_users, batch_items, False)

	# create session to run model
	with tf.Session() as sess:
		sess.run(tf.global_variables_initializer())
		train_iterations = len(data.train_triples) // args.batch_size + 1
		...
		for i in range(1, args.epoch_num+1):
			...
			for j in range(1, train_iterations+1):
				...
				train_feed_dict = {batch_triples: data.train_triples[start:end]}
			...
			if i % 1 == 0:
				...
				valid_feed_dict = {batch_users: data.valid_users, batch_items: data.valid_items}
				...
				test_feed_dict = {batch_users: data.test_users, batch_items: data.test_items}
				...
				if i - bepoch >= 20:
					...
					print(args.logid, ':', args.epoch_num, args.batch_size, args.learn_rate, args.latent_dimension)
